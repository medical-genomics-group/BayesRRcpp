...
```{r,message=F,error=F,warning=F,results=F}
...
Now we run the sbc script for the $\sigma_\beta$ parametrization
```{r,eval=F}
source('./sbc.R')

sbc.result <- sbc(scale(X),scale(Covariates),num.params = 0,sbc.sweeps = 100,posterior.draws = 1000,init.thin = 5,max.thin = 100, target.neff = 900,FUN = BayesRSamplerV2sigmaB)
hist(sbc.result$rank,breaks=100)
```

The same for the $\sigma_G$ parametrization

Now we run the sbc script for the $\sigma_\beta$ parametrization
```{r,eval=F}
source('./sbc.R')

sbc.result <- sbc(scale(X),scale(Covariates),num.params = 100,sbc.sweeps = 1,posterior.draws = 1000,init.thin = 5,max.thin = 100, target.neff = 900,FUN = BayesRSamplerV2)
