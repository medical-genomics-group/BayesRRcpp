...
public:
  ...
  Matrix<Scalar,Dynamic,Dynamic> samples(int nn)
  {
    ...
    return  _transform.solve( Matrix<Scalar,Dynamic,-1>::NullaryExpr(_covar.rows(),100,randN))+_transform.solve(_mean);
  }
};
...
public:
  ...
  void setCovar(const Matrix<Scalar,Dynamic,1>& y,const Matrix<Scalar,Dynamic,Dynamic>& x,const VectorXd d)
  {
    ...
    _covar =u + d.asDiagonal()*x.transpose()*(sumIdentity(x*d.asDiagonal()*x.transpose())).lu().solve( y-x*u-(Matrix<Scalar,Dynamic,-1>::NullaryExpr(x.rows(),100,randN)).matrix());
  }
