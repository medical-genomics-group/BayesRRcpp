...
void BayesRSamplerV2QR(std::string outputFile, int seed, int max_iterations, int burn_in, int thinning, Eigen::MatrixXd X, Eigen::VectorXd Y,double sigma0, double v0E, double s02E, double v0G, double s02G,Eigen::VectorXd cva) {
  ...
  MatrixXd thinQ(MatrixXd::Identity(N,M)),R,IR;
  HouseholderQR<MatrixXd> qr(X);

  thinQ = qr.householderQ()* thinQ*N;
  FullPivLU<MatrixXd>lu_decomp(X);
  int Rank = lu_decomp.rank();                 //retrieve rank of matrix
   R = qr.matrixQR().topLeftCorner(Rank, Rank).template triangularView<Upper>();
   IR=(R).inverse();
   std::cout<< IR.rows()<< " " << IR.cols()<<"\n";
   //std::cout<< IR.row(0)<<"\n";
...
#pragma omp sections
{

  {
    ...
    epsilon= Y.array() - mu - (thinQ*beta).array();
    ...
    for(int iteration=0; iteration < max_iterations; iteration++){
      ...
      if(iteration>0)
        ...
        for(int j=0; j < M; j++){
          ...
          y_tilde= epsilon.array()+(thinQ.col(marker)*beta(marker,0)).array();//now y_tilde= Y-mu-X*beta+ X.col(marker)*beta(marker)_old
          ...
          denom=thinQ.col(marker).squaredNorm()+(sigmaE/sigmaG)*cVaI.segment(1,(K-1)).array();
          //muk for the other components is computed according to equaitons
          muk.segment(1,(K-1))= (thinQ.col(marker).cwiseProduct(y_tilde)).sum()/denom.array();
          ...
          */
          ...
          logL.segment(1,(K-1))=logL.segment(1,(K-1)).array() - 0.5*((((sigmaG/sigmaE)*(thinQ.col(marker).squaredNorm())*cVa.segment(1,(K-1)).array() + 1).array().log()))+
            0.5*( muk.segment(1,(K-1)).array()*((thinQ.col(marker).cwiseProduct(y_tilde)).sum()))/sigmaE;
          ...
          epsilon=y_tilde-thinQ.col(marker)*beta(marker,0);//now epsilon contains Y-mu - X*beta+ X.col(marker)*beta(marker)_old- X.col(marker)*beta(marker)_new
        ...
        if(iteration >= burn_in)
        {
          if(iteration % thinning == 0){
            ...
            sample<< iteration,mu,N*(IR*beta),sigmaE,sigmaG,components;
            q.enqueue(sample);
          }
