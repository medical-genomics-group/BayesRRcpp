...
#pragma omp sections
{
  //begin producer
  {
    ...
    MatrixXd xtX(M,M); //potential field
    VectorXd sigmaL(M); //component specific variances;
    ...
    for(int iteration=0; iteration < max_iterations; iteration++){
      ...
      if(iteration>0)
          ...
          for(int k=0;k<K;k++){
            if(p<=acum){
              ...
              //  beta(marker,0)=norm_rng(muk[k],sigmaE/denom[k-1]);
                 sigmaL(marker)=sigmaG*cVa(k);
                 nonZero.push_back(marker);
              }
              v[k]+=1.0;
              components[marker]=k;
              break;
        ...
        beta(nonZero)=mvnCoef_rng(1,
                    X(nonZero).transpose()*(Y.array()-mu),
                    xtX(nonZero,nonZero),
                    sigmaL(nonZero));
        m0=M-v[0];
        ...
        sigmaE=inv_scaled_chisq_rng(v0E+N,((Y-X(nonZero)*beta(nonZero)).squaredNorm()+v0E*s02E)/(v0E+N));
